        ...
        <div class="p-4 border-dashed rounded-lg dark:border-gray-700 mt-14">
            ...
            <form action="{{ route('reparations.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="description" name="description" class="w-full p-2 border rounded-lg" value="{{ old('description') }}">
                </div>
                <div class="mb-4">
                    <label for="status" class="block text-gray-700 dark:text-white">Status:</label>
                    <input type="text" id="status" name="status" class="w-full p-2 border rounded-lg" value="{{ old('status') }}">
                </div>
                <div class="mb-4">
                    <label for="startDate" class="block text-gray-700 dark:text-white">Start Date:</label>
                    <input type="date" id="startDate" name="startDate" class="w-full p-2 border rounded-lg" value="{{ old('startDate') }}">
                </div>
                <div class="mb-4">
                    <label for="endDate" class="block text-gray-700 dark:text-white">End Date:</label>
                    <input type="date" id="endDate" name="endDate" class="w-full p-2 border rounded-lg" value="{{ old('endDate') }}">
                </div>
                <div class="mb-4">
                    <label for="mechanicNotes" class="block text-gray-700 dark:text-white">Mechanic Notes:</label>
                    <textarea id="mechanicNotes" name="mechanicNotes" class="w-full p-2 border rounded-lg">{{ old('mechanicNotes') }}</textarea>
                </div>
                <div class="mb-4">
                    <label for="clientNotes" class="block text-gray-700 dark:text-white">Client Notes:</label>
                    <textarea id="clientNotes" name="clientNotes" class="w-full p-2 border rounded-lg">{{ old('clientNotes') }}</textarea>
                </div>
                <div class="mb-4">
                    <label for="vehicleID" class="block text-gray-700 dark:text-white">Vehicle:</label>
                    <select id="vehicleID" name="vehicleID" class="w-full p-2 border rounded-lg">
                        @foreach ($vehicule as $v)
                            <option value="{{ $v->id }}">{{ $v->Marque }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    <button type="submit" class="w-full bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg">Create</button>
                </div>
            </form>
        </div>
    </div>
