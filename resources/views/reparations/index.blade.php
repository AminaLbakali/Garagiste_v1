    ...
    <nav class="fixed top-0 z-50 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
      ...
      <div class="p-4 sm:ml-64">
        ...
        <div class="container mx-auto py-8">
            <form action="{{ route('reparations.create') }}" method="GET" class="mb-4">
                <button type="submit" class="w-auto bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:shadow-outline transform transition-all duration-150 ease-in-out">
                    <i class="fa-solid fa-plus"></i> Ajoute une nouvelle reparation
                </button>
            </form>

            <div class="">
                <table id="reparations-table" class="w-full table-auto rounded-lg">
                    <thead>
                        <tr class="bg-gray-500 text-white bg-indigo-700 rounded">
                            <th class="px-4 py-2 rounded-l-lg">Description</th>
                            <th class="px-4 py-2">Status</th>
                            <th class="px-4 py-2">Start Date</th>
                            <th class="px-4 py-2">End Date</th>
                            <th class="px-4 py-2">Mechanic Notes</th>
                            <th class="px-4 py-2">Client Notes</th>
                            <th class="px-4 py-2 rounded-r-lg">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($reparations as $index => $reparation)
                            <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-300">
                                <td class="px-4 py-2 rounded-l-lg">{{ $reparation->description }}</td>
                                <td class="px-4 py-2">{{ $reparation->status }}</td>
                                <td class="px-4 py-2">{{ $reparation->startDate }}</td>
                                <td class="px-4 py-2">{{ $reparation->endDate }}</td>
                                <td class="px-4 py-2">{{ $reparation->mechanicNotes }}</td>
                                <td class="px-4 py-2">{{ $reparation->clientNotes }}</td>
                                <td class="px-4 py-2 rounded-r-lg">
                                    <a href="{{ route('reparations.edit', $reparation->id) }}" class="text-blue-600 hover:underline">
