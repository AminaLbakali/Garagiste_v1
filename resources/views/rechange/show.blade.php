   ...
   <nav class="fixed top-0 z-50 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <h1 class="text-4xl font-bold text-indigo-800 mb-4">{{ $piecesRechange->nom }}</h1>
            <p class="text-lg text-gray-700 mb-2">Référence: <span class="text-indigo-700 font-medium">{{ $piecesRechange->reference }}</span></p>
            <p class="text-lg text-gray-700 mb-2">Fournisseur: <span class="text-indigo-700 font-medium">{{ $piecesRechange->fournisseur }}</span></p>
            <p class="text-lg text-gray-700 mb-2">Prix: <span class="text-indigo-700 font-medium">{{ $piecesRechange->prix }}</span></p>
            <p class="text-lg text-gray-700 mb-4">Stock: <span class="text-indigo-700 font-medium">{{ $piecesRechange->stock }}</span></p>
            <a href="{{ route('pieces-rechange.edit', $piecesRechange->id) }}" class="inline-block bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-600 transition-colors duration-200 shadow-md mb-4">Modifier</a>
            <form action="{{ route('pieces-rechange.destroy', $piecesRechange->id) }}" method="POST" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors duration-200 shadow-md">Supprimer</button>
                <a href="{{ route('rechange.index') }}" class="inline-block bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-colors duration-200 shadow-md">Annuler</a>
            </form>
         </div>      
